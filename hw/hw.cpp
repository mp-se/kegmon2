    ...
    if (myInfluxTimer.hasExpired() && myConfig.hasTargetInfluxDb2()) {
      ...
      for (int i = 0; i < MAX_SCALES; i++) {
        ...
        if (lastPourVolume > 0.0f) {
          snprintf(&buf[0], sizeof(buf), "last_pour_vol%i=%f,", i + 1, lastPourVolume);
          s = s + &buf[0];
        }
      ...
      while (myChangeDetection.getNextEvent(event)) {
        ...
        if (event.type == ChangeDetectionEventType::STABLE_LEVEL) {
          snprintf(&buf[0], sizeof(buf),
                   ...
                   event.stable.stableWeightKg,
                   WeightVolumeConverter(event.unitIndex).weightToVolume(event.stable.stableWeightKg),
